...
//{{{
Job::Job(const std::experimental::filesystem::path& jobfile, ev::loop_ref& loop) : jobname(jobfile.filename()), jobfile(jobfile), write_time_timer(loop)
{
...
//
//{{{
Job::Job(Job && other) noexcept : jobname(other.jobname), jobfile(other.jobfile), settings(std::move(other.settings)), write_time_timer(other.write_time_timer.loop)
{
