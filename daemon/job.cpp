...
uint32_t time_string_to_seconds(std::string input)
{
	...
	return seconds ? seconds : default_timefile_granularity;
}
//}}}
	...
	jobname(jobfile_path.stem())
{
	debug<<"Creating job \""<<jobname<<"\" from "<<jobfile_path<<std::endl;
		...
		if (reader.ParseError() < 0)
		{
			...
			notify_critical<<"Cannot parse job fiel for job"<<jobname<<std::endl;
		...
		timefile_granularity = time_string_to_seconds(reader.Get("timefile", "granularity", "1h"));
