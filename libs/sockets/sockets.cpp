...
int open_socket(const std::string& socket_path, bool as_server)
{
	int fd;
	...
	// Since Strings that begin with a null character are difficult to handle, use % instead
	// and translate @ to the null character here.
	if( addr.sun_path[0] == '@' ) addr.sun_path[0] = '\0';

	if( as_server )
	{
		if( bind(fd, (static_cast < struct sockaddr* > (static_cast < void* > (&addr))), sizeof(addr.sun_family)+socket_path.length()) == -1 )
		{
	...
	{
		if( connect(fd, (static_cast < struct sockaddr* > (static_cast < void* > (&addr))), sizeof(addr.sun_family)+socket_path.length()) == -1 )
		{
